...
const UpdateLabTest = () => {
  ...
  // Define updateTest function

  useEffect(() => {
    const queryParameters = new URLSearchParams(window.location.search)
    setId(queryParameters.get("id"))
    setTestName(queryParameters.get("test_name"))
    setPurpose(queryParameters.get("purpose"))
    setPreparation(queryParameters.get("preparation"))
    setTiming(queryParameters.get("timing"))
    setPrice(queryParameters.get("price"))
  }, []);

  
  const UpdateLabTest = async (event) => {
    ...
    //frontend to backend 
    await axios.put('http://localhost:5001/api/v1/lab-tests', data).then((res) => {
        ...
        <Row className="mt-5">
          <Col md="4" style={{background:"white",boxShadow:"0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19)",borderRadius:"25px"}}>
            <div className="padding-box">
              ...
              <Button variant="info" style={{width:"-moz-available"}}>Update Test</Button><br /><br />
          ...
          <Col md="7" style={{background:"white",boxShadow:"0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19)",borderRadius:"25px"}}>
            <div className="padding-box">
              ...
              <Form className="mt-3" type="post">
                ...
                <Button onClick={(e) => UpdateLabTest(e)} as="input" type="submit" value="Submit" />
                  </Form>
            </div>
          </Col>
        </Row>
